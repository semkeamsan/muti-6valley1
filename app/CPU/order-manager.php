...
class OrderManager
{
    ...
    {
        $order = Order::with(['payment_images'])->where(['id' => $order_id])->first();
        $payment_images = OrderPaymentImageResource::collection($order['payment_images']);
        unset($order['payment_images']);
        $order['payment_images'] = $payment_images;
    ...
    {
        $order_id = 100000 + Order::query()->count() + 1;
        ...
        if ($data['payment_image_url'] && $data['payment_method_id']) {

            $image = collect(explode('/', $data['payment_image_url']))->last();

            $file = move_image('temp/' . $image, 'payment-images/', $image);

            $payment_method = PaymentMethod::query()->find($data['payment_method_id']);

            PaymentImage::query()->create([
                'order_id' => $order_id,
                'image' => $file,
