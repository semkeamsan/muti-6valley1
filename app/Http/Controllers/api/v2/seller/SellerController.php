...
class SellerController extends Controller
{
    public function shop_info(Request $request)
    {
        ...
        $shop['country_code'] = $phone['country_code'];
        $shop['national_number'] = $phone['national_number'];
    ...
    public function seller_info(Request $request)
    {
        ...
        $seller = Seller::query()
            ->with(['wallet'])
            ->withCount(['product', 'orders'])
            ->where(['id' => $seller['id']])
            ->first();

        $phone = format_phone($seller->phone);
        $seller['phone'] = $phone['phone'];
        $seller['country_code'] = $phone['country_code'];
        $seller['national_number'] = $phone['national_number'];
    ...
    public function shop_info_update(Request $request)
    {
        ...
        if ($request['contact']) {
            $phone = validate_phone($request['contact']);
            $request->merge(['contact' => $phone]);
        }
